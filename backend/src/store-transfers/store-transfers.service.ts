...
export class StoreTransfersService {
  ...
  private toTransfer(doc: any) {
    ...
    return {
      ...
      fromStore: fromStore?.name ? { name: fromStore.name, location: fromStore.location } : undefined,
      toStoreId: (toStore?._id || toStore)?.toString?.(),
      toStore: toStore?.name ? { name: toStore.name, location: toStore.location } : undefined,
      status: o.status,
      notes: o.notes,
      lines,
